...
router.post("/", restricted, checkRole("fundraiser"), (req, res) => {
	if (!req.body.pname || !req.body.goal || !req.body.fundraiser) {
		res.json({
			msg: "pname, goal, and fundraiser are required",
		});
	} else {
...
router.delete("/:id", restricted, checkRole("fundraiser"), (req, res) => {
	Projects.remove(req.params.id)
		.then((count) => {
			if (count > 0) res.status(200).json({ msg: "successfully removed" });
...
router.put("/:id", restricted, checkRole("fundraiser"), (req, res) => {
	if (!req.body) {
		...
		Projects.modify(req.params.id, req.body)
			.then((count) => {
				if (count > 0) res.status(200).json({ msg: "successfully changed" });
				else res.status(400).json({ msg: "error" });
			})
			.catch((err) => {
				res.status(500).json({ msg: err.message });
